...
def convert(names_file: str, nums_file: str, result_file: str) -> None:
    with(open(names_file, 'r', encoding='utf-8') as f_names,
        ...
        for i in range(min(len(data_nums), len(data_names))):
            ...
            if mult_res < 0:
                res = data_names[i].lower() + ' ' + str(abs(mult_res))
            else:
                res = data_names[i].upper() + ' ' + str(round(mult_res))
            f_res.write(res + '\n')
        ...
        for i in range(len_dif):
            if is_long_names:
                ...
                mult_res = int_value * float_value
                if mult_res < 0:
                    res = long[str_n + i].lower() + ' ' + str(abs(mult_res))
                else:
                    res = long[str_n + i].upper() + ' ' + str(round(mult_res))
            else:
                ...
                if mult_res > 0:
                    ...
                    res = short[i].upper() + ' ' + str(round(mult_res))
            f_res.write(res + '\n')

if __name__ == '__main__':
    convert('t2.txt', 't1.txt', 't3.txt')
